...
// NEW EMPLOYEE 
 ...
 $Description = mysqli_real_escape_string($connection, $_POST['content']);
	

		// form validation: ensure that the form is correctly filled
		if (empty($first_name)) { array_push($errors, "Date is required"); }
		if (empty($last_name)) { array_push($errors, "Title is required"); }
		if (empty($email)) { array_push($errors, "Content is required"); }

		// register user if there are no errors in the form
		if (count($errors) == 0) {
  
			$query = "INSERT INTO `employee` (first_name, last_name, email, telephone, image_url, job, Description) 
					  VALUES('$first_name', '$last_name', '$email', '$telephone', '$image_url', '$job', '$Description')";
			mysqli_query($connection, $query);
  $message = "Record saved Successfully";
	}

	}
$result = mysqli_query($connection,"SELECT * FROM employee WHERE id='" . $_GET['id'] . "'");
...
<form name="frmUser" method="post" action="">
    ...
    <div class="w3-bar w3-light-gray">
        <div class="input-group">
            <button type="submit" name="new_employee" value="Sauvegarder" class="btn">Sauvegarder</button>
            <button class="btn"><a style="text-decoration: none;" href="employees.php">Retourner à la liste</a></button>
